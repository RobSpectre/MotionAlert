...
class MotionAlert(object):
    ...
    def send(self):
        ...
        # Try to get the most recent image from the camera.
        image_file_path = \
            self.get_latest_image_from_directory(self.motion_target_dir)

        # Try to upload that image to S3.
        if image_file_path:
            s3_key = self.upload_image_to_s3(image_file_path,
                                                self.s3_bucket,
                                                self.timestamp)
        else:
    ...
    def upload_image_to_s3(self, image_file_path, bucket_name, key_name):
        ...
        try:
            # Create a new key - key_name is supposed to be the timestamp from
            # Motion.
            key = Key(bucket)
            key.key = key_name
